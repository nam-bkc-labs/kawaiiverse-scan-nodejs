  ...
  try {
    ...
    for (let k = 0; k < tokenData.length; k++) {
      ...
      if (tokenData[k].type === 'erc1155') {
        const KawaiiCoreContract = new web3.eth.Contract(abiKawaiiCore, tokenData[k].hash.toLowerCase());
      ...
      if (tokenData[k].type === 'erc721') {
        const Kawaii721Contract = new web3.eth.Contract(abiKawaii721, tokenData[k].hash);
        for (let i = 0; i < accountEth.length; i++) {
          const isCheck = await Kawaii721Contract.methods.balanceOf(accountEth[i].hash).call();
          if (Number(isCheck) > 0) {
            total++;
          }
        }
      ...
      if (tokenData[k].type === 'erc20') {
        ...
        const ERC20Contract = new web3.eth.Contract(abiERC20, tokenData[k].hash);
        for (let i = 0; i < accountEth.length; i++) {
          const isCheck = await ERC20Contract.methods.balanceOf(accountEth[i].hash).call();
          if (Number(isCheck) > 0) {
            total++;
          }
        }
    ...
    console.log('sleep 2h from', new Date());
    setTimeout(run, 72e5);
  } catch (e) {
    console.log(e);
