...
const erc1155DB = require('../models/tx_erc1155');
...
async function run() {
  ...
  for (let i = 0; i < erc1155Data.length; i++) {
    console.log(i);
    const txReceipt = await web3Service.GetTransactionReceipt(erc1155Data[i].tx_hash);
    const logs = txReceipt.txEthReceiptData.logs;
    for (let j = 0; j < logs.length; j++) {
      const block = await web3Service.GetBlock(txReceipt.txEthReceiptData.blockNumber);
      if (logs[j].topics[0] === '0xc3d58168c5ae7397731d063d5bbf3d657854427343f4c083240f7aacaa2d0f62') {
        await erc1155DB.update({
